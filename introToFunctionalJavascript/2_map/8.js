...
// Clean up the data so it is more readable. Write a function that returns an array of spell objects, with each object containing the spell's name, description, and duration:
...
const spellObjects = arr => {
  return arr.map(obj => {
    return { name: obj.name, description: obj.desc[0], duration: obj.duration }
  })
}

console.log(spellObjects(spells));